...
export default function RetirarModal({ onClose }: { onClose: () => void }) {
  ...
  // PASO 1: Consultar el saldo del usuario
  ...
  // PASO 2: Confirmar el retiro
  const handleRetirar = async () => {
    const monto = parseFloat(amountToWithdraw);

    if (monto > (balance || 0)) {
      return Swal.fire({ icon: 'warning', title: 'Saldo Insuficiente', text: 'Estás intentando retirar más de lo que tiene el jugador.' });
    }

    setLoading(true);
    Swal.fire({
      title: 'Procesando retiro...',
      text: `Extrayendo $${monto} de ${username}`,
      allowOutsideClick: false,
      didOpen: () => Swal.showLoading()
    });
    ...
    try {
      ...
      if (res.ok) {
        // 👇 ALERTA DE ÉXITO CON EL NUEVO SALDO EN FORMATO TICKET
        Swal.fire({
          icon: 'success',
          ...
          html: `
            ...
            <div style="background: #1f2937; padding: 15px; border-radius: 15px; border: 1px solid #374151;">
              ...
              <strong style="color: #4ade80; font-size: 28px; font-family: monospace;">$${parseFloat(data.newBalance).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
            </div>
          `,
          confirmButtonColor: '#dc2626',
          confirmButtonText: 'Entendido'
        }).then(() => {
          onClose(); // Cerramos el modal cuando el cajero le da a "Entendido"
        });
      } else {
        Swal.fire({ icon: 'error', title: 'Zeus rechazó la operación', text: data.error });
      }
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo procesar el retiro' });
    } finally {
      setLoading(false);
    }
  };
        ...
        {step === 1 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className="flex-[2] bg-red-600 hover:bg-red-700 py-4 rounded-2xl font-black text-xs uppercase transition-all"
                >
          ...
          <div className="animate-in zoom-in-95 duration-300">
            ...
            <div className="bg-gray-800 p-6 rounded-2xl text-center mb-6 border border-gray-700">
                ...
                <p className="text-green-400 font-mono font-black text-3xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-950 border border-gray-700 p-4 rounded-2xl text-white font-bold outline-none focus:ring-2 focus:ring-red-500 text-center text-xl"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button 
                  ...
                  disabled={!amountToWithdraw || loading} 
                  className="flex-[2] bg-red-600 hover:bg-red-700 py-4 rounded-2xl font-black text-xs uppercase transition-all shadow-lg shadow-red-900/30"
                >
