...
export default function CargarModal({ transfer, onClose, onSuccess }: Props) {
  ...
  // --- SEGURO PARA DESCONEXIONES O CIERRE DE PESTAÑA ---
  useEffect(() => {
    const liberarCargaAutomatico = () => {
      // Usamos keepalive para que la petición viaje aunque se cierre la pestaña
      fetch(`/api/transferencias/${transfer._id}/liberar`, { 
        method: 'POST',
        keepalive: true 
      });
    };

    window.addEventListener('beforeunload', liberarCargaAutomatico);

    return () => {
      window.removeEventListener('beforeunload', liberarCargaAutomatico);
    };
  }, [transfer._id]);

  const calcularBonoFinal = () => {
    const v = parseFloat(valorBono) || 0;
    return tipoBono === 'porcentaje' ? (transfer.monto * v) / 100 : v;
  };

  const bonoCalculado = calcularBonoFinal();
  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 backdrop-blur-md">
      <div className="bg-gray-900 border border-gray-800 p-8 rounded-[32px] w-full max-w-md shadow-2xl">
        ...
        <div className="bg-gray-800/50 p-4 rounded-2xl mb-6 border border-gray-800">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-gray-800/50 rounded-2xl border border-gray-800">
            ...
            {conBono && (
                <input type="number" value={valorBono} onChange={e => setValorBono(e.target.value)} placeholder={tipoBono === 'porcentaje' ? "% Extra" : "Monto Extra"} className="w-full bg-gray-900 border border-gray-700 p-3 rounded-xl text-white font-bold"/>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button 
              ...
              className="flex-[2] bg-green-600 hover:bg-green-700 py-4 rounded-2xl font-black text-xs uppercase disabled:opacity-20 transition-all"
            >
