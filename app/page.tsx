              ...
              return (
                <div key={t._id} className={`bg-gray-900 border p-7 rounded-[32px] transition-all shadow-xl ${isMine ? 'border-yellow-500 bg-yellow-500/5 shadow-yellow-900/10' : isTakenByOther ? 'border-purple-500/20 opacity-40' : 'border-gray-800 hover:border-blue-500/40'}`}>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {t.estado === 'EN_PROCESO' && <span className={`text-[9px] px-2 py-1 rounded-full font-black uppercase ${isMine ? 'bg-yellow-600 text-white' : 'bg-purple-600 text-white'}`}>{isMine ? 'EN TU MANO' : t.cajeroAsignado?.nombre || 'OCUPADA'}</span>}
                  </div>
                  <div className="mb-6">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-800">
                  {paginatedRealizadas.map((t: any) => {
                    const total = (t.monto || 0) + (t.montoBono || 0);
                    return (
                      <tr key={t._id} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="p-5">
                          ...
                          {t.coelsaCode && !['CANAL', 'AGENDAMIENTO', 'INSTAGRAM'].includes(t.remitente) && <p className="text-[8px] text-gray-600 font-mono mt-1 uppercase">{t.coelsaCode}</p>}
                        </td>
                        <td className="p-5 font-mono text-gray-400 text-sm">${(t.monto || 0).toLocaleString()}</td>
